...
#define INITWAIT    2       /* Seconds to show initialisation messages before going visual */
#define GRIDMARK    '+'
...
//  signal(SIGWINCH, sig_winch); // while debugging
  ...
  printf("Data storage for history log initialised (%lu bytes)\n", sizeof(passdata)*HISTLOG*sizeof(pingdata)*ntargets);
...
void print_packet(char *packet, int len, struct sockaddr_in *from) {
  ...
  //if (icp->icmp_id != pid) return; // FIXME:
...
void send_ping(target *t) {
  ...
  sendto(sock, packet, len, 0, (struct sockaddr *)t->addr, sizeof(struct sockaddr));
}
...
void start_curses(void) {
  ...
  hostinfo = newwin(13, 51, (rows-13)/2, (cols-51)/2);
